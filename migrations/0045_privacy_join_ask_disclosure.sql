...
UPDATE page_content
SET content = '<h2>What we collect</h2><ul><li>Join / Login: first name, last name, optional screen name, email address, and optional email opt-in for updates.</li><li>Ask a Question: first name, last name, optional screen name, email address, and the question text you submit for moderator review.</li><li>Member creation and verification: when you Join or Ask with a new email, we may create a member record and send a welcome or transactional confirmation email related to that request.</li><li>Session authentication: a short-lived session cookie used to keep you signed in to Fan Club member surfaces after Login. We do not use magic-link authentication.</li><li>Library submissions: the content you submit, your provided name/email, and timestamps.</li><li>Photo submissions (when enabled): the media URL, description/caption text you provide, and timestamps.</li><li>Basic technical logs (for example request timing/errors) used for reliability, security, and rate limiting.</li></ul><h2>How we use it</h2><ul><li>To operate Join/Login and member access, including session cookies for authenticated Fan Club pages.</li><li>To review and reply to Ask questions, and to add approved answers to the public FAQ when appropriate.</li><li>To send email updates only when you explicitly opt in via Join. First-time Join or Ask requests may also receive a welcome email; other non-opted-in mail is limited to transactional confirmations related to that request.</li><li>To publish and manage user submissions (Library and photo/media captions).</li><li>To keep the site secure and functioning (rate limiting, troubleshooting).</li></ul><h2>What we do not do</h2><p>We do not sell personal information. We do not use magic-link authentication or verification.</p><h2>Removal requests</h2><p>To request removal of your email or a submission, contact: <strong>admin@lougehrigfanclub.com</strong>.</p><p>This policy will be refined as features expand. When we add new data collection, we will update this page.</p>'
WHERE slug = '/privacy'
  ...
  AND (
    ...
    OR content LIKE '%strictly transactional confirmations%'
    OR (
      content LIKE '%optional screen name%'
      AND content NOT LIKE '%magic-link%'
    )
  );
