const TEAM_QUEUE_ORDER = [
  ...
  { id: 'pmoActive', title: 'PMO Active', teamLabel: 'team:pmo', lifecycleLabel: 'pmo:active' },
  { id: 'engineering', title: 'Engineering', teamLabel: 'team:engineering' },
  { id: 'governance', title: 'Governance', teamLabel: 'team:governance' },
  { id: 'pmoPipeline', title: 'PMO Pipeline', teamLabel: 'team:pmo', lifecycleLabel: 'pmo:pipeline' }
];
...
export function buildTeamQueueCounts(issues, { owner, repo } = {}) {
  const counts = Object.fromEntries(TEAM_QUEUE_ORDER.map((queue) => [queue.id, 0]));
  ...
  return {
    ...
    queues: TEAM_QUEUE_ORDER.map((queue) => ({
      id: queue.id,
      title: queue.title,
