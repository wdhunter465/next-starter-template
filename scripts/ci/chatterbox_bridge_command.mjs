...
// Comments posted through this repo's own tooling get a "Generated by
// Claude Code" (or similar) attribution footer auto-appended after the
// text a human or agent actually typed. Left alone, that footer becomes
// part of whatever free-form body text follows it. Strip it before the
// body is used for anything.
function stripAttributionFooter(text) {
  // The leading newline is optional: if the real body content was empty
  // (e.g. a bare "body:" line) and only the footer followed, an earlier
  // .trim() on the joined lines can eat the newline that would otherwise
  // separate the (empty) body from the footer, leaving the footer at the
  // very start of the string with nothing before it to match "\n+"
  // against (Copilot finding, PR #3534).
  return text.replace(/\n*---\s*\n+_Generated by \[[^\]]+\]\([^)]*\)_\s*$/i, '').trimEnd();
}
...
function parseCommand(body) {
  ...
  for (let i = commandLineIndex + 1; i < lines.length; i++) {
    ...
    // Not a recognized "key: value" line. Blank lines between fields are
    // just formatting -- skip them. The first non-blank, non-key line
    // implicitly starts the body, so a human typing a question naturally
    // after `participant: x` doesn't need to know the `body:` label
    // exists (Bill's feedback, 2026-08-15: requiring it was "messy" and
    // "very difficult for a human to join and participate").
    if (line.trim() === '') continue;
    bodyLines = [line];
  }

  if (bodyLines !== null) {
    fields.body = stripAttributionFooter(bodyLines.join('\n').trim());
  }
