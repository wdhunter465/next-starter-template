#!/usr/bin/env node
// #3415 (Chatterbox) — resolve the live Cloudflare Pages deployment URL for
// a given branch via the Cloudflare API, using the same CLOUDFLARE_API_TOKEN
// / CLOUDFLARE_ACCOUNT_ID secrets already used by the existing D1 write
// tooling. Read-only: lists deployments, never creates or mutates one.
//
