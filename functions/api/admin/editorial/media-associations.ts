...
async function persistRenditions(context: {
  request: Request;
  ...
  const tables = await requireTables(db, [
    "content_inventory",
  ...
  const associatedIds = new Set(
    (associated.get(storyId) || []).map((row) => asInt((row as any).media_id)),
  );
  ...
  for (const item of items) {
    ...
    if (!associatedIds.has(mediaId)) {
      return jsonResponse(
        { ok: false, error: `media_id ${mediaId} is not associated with story ${storyId}.` },
        400,
      );
    }
