 ...
 */
export async function composeClubHomeSelection(
  db: any,
  asOfDate = new Date(),
): Promise<ComposedClubHomeSelection | null> {
...
export async function listClubHomeEditionPlacements(
  db: any,
  editionId: number,
): Promise<ClubHomeEditionPlacementRecord[]> {
  const rows = await db
    .prepare(
...
async function payloadFromEdition(
  ...
  const spotlightPlacement = placements.find(
    (row) => row.zone_id === CLUB_HOME_PLACEMENT_ZONES.archiveSpotlight,
  );
  const mediaPlacement = placements.find((row) => row.zone_id === CLUB_HOME_PLACEMENT_ZONES.mediaFeature);

  const leadRow =
    leadPlacement?.story_id != null ? await loadEligibleStoryById(db, leadPlacement.story_id) : null;
  ...
  const spotlightRow =
    spotlightPlacement?.story_id != null
      ? await loadEligibleStoryById(db, spotlightPlacement.story_id)
      : null;
  ...
  try {
    ...
    if (composed.leadRow) {
      placementRows.push({
        ...
        position: 0,
        selection_mode: composed.leadSelectionMode,
        feature_size: mediaFeature
          ? mediaFeature.is_memorabilia
            ? 'memorabilia'
            : 'photo'
          : null,
      });
...
export async function rollbackClubHomeEdition(
  ...
  try {
    ...
    const edition = await db
      .prepare(
        `SELECT id, status FROM content_inventory_club_home_editions WHERE id = ?`,
      )
      .bind(targetId)
      .first();
    if (!edition) {
