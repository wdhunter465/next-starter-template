...
import { putB2Object, requireB2, type B2Bindings } from "./b2";
...
const PUBLIC_IMAGE_ROLES = new Set(["primary_image", "gallery_image", "memorabilia_reference", "supporting_image"]);
...
export async function resolvePhotoByReference(db: any, reference: string): Promise<Record<string, unknown> | null> {
  ...
  const byPhotoId = await db
    .prepare(
      `SELECT id, photo_id, url, title, description, source FROM photos WHERE lower(trim(photo_id)) = lower(trim(?)) AND ${rightsClearedClause()}`,
    )
    .bind(trimmed)
    .first();
  ...
  return db
    .prepare(
      `SELECT id, photo_id, url, title, description, source FROM photos WHERE lower(trim(url)) = lower(trim(?)) AND ${rightsClearedClause()}`,
    )
    .bind(trimmed)
    .first();
}
...
export async function listStoryMediaAssociations(db: any, storyIds: number[]): Promise<Map<number, Array<Record<string, unknown>>>> {
...
export function publicUrlForRenditionKey(env: Record<string, unknown>, cfg: B2Bindings, key: string): string {
...
export async function getMediaRendition(db: any, mediaId: number, size: RenditionSize): Promise<Record<string, unknown> | null> {
  return db
    .prepare(
...
export async function putRenditionObject(cfg: B2Bindings, key: string, bytes: Uint8Array, contentType: string): Promise<void> {
  await putB2Object(cfg, { key, body: bytes, contentType });
}
...
export async function buildAssociationsFromSubmission(
  ...
  const altText = asString(photo.title) || asString(photo.description) || "Historical photo associated with submitted story.";
