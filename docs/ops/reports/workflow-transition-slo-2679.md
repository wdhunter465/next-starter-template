...
| --- | --- | ---: | --- | --- | --- |
| T1 | Wake event → runner delivery ack | ≤ 2 min | Authoritative start is the **later** of (a) the GitHub Issues `labeled` event timestamp that first makes both `agent:cursor` and `handoff:ready` present on the Issue, or (b) the `workflow_dispatch` run `created_at` when wake is manually forced. Collectors must use a single concrete timestamp (event `created_at` or run `created_at`), not a “when labels become jointly true” inference without a recorded event. | End is the `cursor-local-wake.yml` job named **`deliver`** completing with `conclusion=success` (`jobs.deliver` completion time / job `completed_at`). Do not use a generic “cursor-local-wake job” name. | runner/host |
