...
| `/admin/editorial` | Editorial Archive | **Pre-existing, parallel `Submission` review workflow** (`content_inventory` / `submission_queue` schema, migrations 0036–0038, 0042 predecessor era): statuses `pending → triaged → under_review → approved/rejected/merged/purged`, with `triage_flags`, `duplicate_candidate`, `review_notes`, `decision_by/at`, `purge_eligible_at`, `retention_reason`. Functionally adjacent to several #2270 queues (see Work unit 2) but built on a distinct, older schema — **not** the #2270 `candidate_id`/`input_stream` model |
