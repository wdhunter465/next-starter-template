...
- `rights_evidence.conclusion` (the three-value vocabulary: `public_domain_confirmed` / `permission_granted` / `lgfc_member_owned_item_photo`) stays `NULL` until a human writer sets it — no discovery or batch-approval code path sets it automatically. `rights_evidence.evidence_text`/`evidence_url` capture the source's own claim verbatim, kept separate from LGFC's conclusion (migration 0055).
