...
Related Issues: #2784, #2918, #2919, #3076
...
Covers the two administrator-controlled evidence controls built for #2919: `POST /api/admin/editorial/suppress` (content takedown) and `POST /api/admin/member-operations/delete` (member soft-deletion/restore), plus the #3076 session-revoke and login-block follow-on. Both are reversible-by-design (nothing is hard-deleted) and both leave an audit trail. Does not cover a public-facing takedown route (none exists, per the approved decision) or any destructive/hard-deletion path.

## Current known truth

Both controls are additive, admin-only, and reversible; no hard-delete path exists in the repository for either content or member records. Soft-deletion also immediately revokes that member's `member_sessions` rows and blocks `/api/login` while `deleted_at` is set (#3076). Restore does not revive the revoked session IDs; the member must log in again.
...
2. Call `POST /api/admin/member-operations/delete` with an admin token:
   ...
   ```
3. The `members` row is marked `deleted_at`/`deletion_reason`/`deleted_by`; the matching `join_requests` row is marked `deleted_at` too. Neither row is removed. Active `member_sessions` for that email are deleted immediately, so an existing `lgfc_session` cookie no longer authenticates.
4. Reply to the requester confirming the account has been deleted.

### Session revoke and login block (#3076)

While `deleted_at` is set, `/api/login` refuses a new session for that email using the same `404 Email not found.` response as an unknown email (do not add a separate "account deleted" message). Restore does not resurrect the revoked session IDs.

### Recovery

Soft-deletion is reversible. Call the same endpoint with `"action": "restore"` and the member's `email` (no other fields required) to clear the deletion markers on both `members` and `join_requests`. The restored member must log in again to receive a new session; prior session cookies remain invalid.
