...
Root cause: `scripts/ci/issue_accounting.mjs`'s `sourceIssueAccounting()` (which parses the PR **body**) treats *any* body line matching `/^issue\b(?:\s*:)?\s*(.*)$/i` after stripping list/bold markup as a competing canonical source-issue declaration — not only the intended `- **Issue:** #123` line. A prose bullet that merely *starts* with the word "Issue" (colon optional, per the accepted colon-less `Issue #123` format above) is read the same way, and if it happens to mention more than one issue number, the post-merge resolver reports the whole set as ambiguous. `resolveSourceIssueFromPr()`'s separate **title** and **branch** stages (`exactIssueTokens()` and `branchIssueTokens()` respectively) do not use this "starts with Issue" heuristic — they extract every `#123`/issue-URL token (title) or numeric path segment (branch) unconditionally — but they have their own, distinct multiple-token exposure: a title or branch name that happens to contain more than one issue-shaped number is ambiguous the same way, just via token-counting rather than a line-prefix match.
...
- For a remediation PR reopening a post-merge closeout exception's original source Issue, prefer the `<!-- orchestrator-source-issue: 123 -->` hidden marker at the top of the body — `resolveSourceIssueFromPr()` honors it ahead of all body/title/branch scanning, so it is immune to this failure mode regardless of what other issue numbers the body, title, or branch name mention.
- This is a post-merge-only failure mode. As of this writing, the pre-merge workflows nominally covering PR issue accounting do not actively validate it: `gate-ensure-issue.yml` (runs on PR opened/reopened) is a deprecated no-op stub, and `ops-pr-issue-accounting.yml` (`GATE — PR Issue Accounting`) is `workflow_dispatch`-only with no automatic trigger and a stub `pwd` job. So a clean-looking pre-merge PR provides no evidence either way about this failure mode — there is currently no automatic pre-merge check to have passed or failed.
